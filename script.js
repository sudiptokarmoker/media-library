const inpFile = document.getElementById("fileInput"), btnUpload = document.getElementById("btnUpload"), canvas = document.getElementById('canvas'), ctx = canvas.getContext('2d');
...
function removeCurrentImage(fileName){
    if (confirm("Are you sure you want to delete this image from gallery directory")) {
        ...
        var fd = new FormData(document.forms["frmCanvasData"]);
        var xhr = new XMLHttpRequest();
        //xhr.open('POST', 'delete_files.php', true); 
...
/*
function applyFlip(img, flipType){
...
function applyRotate(img){

}

function initCropFullFeatures(){
